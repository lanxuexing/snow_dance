...
- **🎨 极致 UI/UX**
  - **Apple 流体交互与设计工程**：遵循 Apple 官方流体交互规范与 Emil Kowalski 设计工程哲学，文章卡片与导航添加 `scale(0.97)` 点击按压反馈，`Cmd + K` 搜索弹窗以 `scale(0.95)` 平滑渐变展开，首页列表加载呈现 40ms 交错淡入位移。
  - **全平台主题引擎与 0ms 启动屏**：结合 `SharedPreferences` 与 Web 原生 `localStorage` 实现全平台主题硬核双向持久化；`main()` 阶段同步注入主题，HTML 启动 Loading 屏与移动端系统状态栏/导航栏全透明沉浸适配。
  - **动态旋转雪花 Logo**：全矢量绘制的高级感雪花图标，带悬浮慢速旋转与微拟态发光交互，全局应用于首屏加载器、标题栏及侧边抽屉中。
  - **玻璃拟态设计**：磨砂玻璃特效、细腻的渐变和深色/浅色模式支持。
...
- **📝 高级 Markdown 引擎**
  - **代码块自由划选 (`SelectableHighlightView`)**：技术文章内的代码块现在支持原生文本划选与复制，大幅提升阅读与学习效率。
  - **语法高亮**：代码块自动检测语言并高亮显示。
  - **自动目录 (ToC)**：支持滚动监听（Scroll-spy）的高亮目录，自动过滤代码块与 Frontmatter 注释干扰。
