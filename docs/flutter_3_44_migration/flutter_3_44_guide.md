...
```dart
// ❌ 传统写法：定义临时 Class 或 Map<String, dynamic>
Map<String, dynamic> getCategoryInfo(String category) {
  return {'icon': Icons.article_outlined, 'color': Colors.green};
}

// ✅ 推荐写法：使用 Record 直接返回元组 (IconData, Color)
...
```dart
// ❌ 传统写法：订阅 MediaQueryData 全量属性，容易造成无关重绘
final isMobile = MediaQuery.of(context).size.width < 800;

// ✅ 官方推荐高级写法：细粒度订阅，只在屏幕宽度改变时重绘
...
```dart
// ❌ 传统写法：setState() 导致整个父级组件树全量 build
